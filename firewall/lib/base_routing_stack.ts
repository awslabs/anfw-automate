...
export class BaseRoutingStack extends TaggedStack {
  constructor(scope: Construct, id: string, props: BaseRoutingProps) {
    ...
    for (const az of Object.values(props.azIds)) {
      ...
      const firewallRouteTable = new ec2.CfnRouteTable(
        this,
        `FirewallRouteTable_${azSuffix.toUpperCase()}`,
        {
          vpcId: props.vpcId,
          tags: [
            { key: 'Name', value: `rtb.${props.namePrefix}.nfw.10${azSuffix}.${props.stage}` },
          ],
        }
      );
      ...
      new ec2.CfnSubnetRouteTableAssociation(
        this,
        `FirewallRouteTableAssociation_${azSuffix.toUpperCase()}`,
        {
      ...
      const publicRouteTable = new ec2.CfnRouteTable(
        this,
        `PublicRouteTable_${azSuffix.toUpperCase()}`,
        {
          vpcId: props.vpcId,
          tags: [
            { key: 'Name', value: `rtb.${props.namePrefix}.ngw.11${azSuffix}.${props.stage}` },
          ],
        }
      );
      ...
      new ec2.CfnSubnetRouteTableAssociation(
        this,
        `PublicRouteTableAssociation_${azSuffix.toUpperCase()}`,
        {
      ...
      new ec2.CfnSubnetRouteTableAssociation(
        this,
        `TGWRouteTableAssociation_${azSuffix.toUpperCase()}`,
        {
