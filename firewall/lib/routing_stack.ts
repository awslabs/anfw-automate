...
export class RoutingStack extends TaggedStack {
  constructor(scope: Construct, id: string, props: RoutingStackProps) {
    ...
    deleteRoutesLambda.addToRolePolicy(
    ...
    fetchAnfwVpceLambda.addToRolePolicy(
      new iam.PolicyStatement({
    ...
    for (const azKey in props.azIds) {
      ...
      const firewallDefaultRoute = new ec2.CfnRoute(
        this,
        `FirewallDefaultRoute${azSuffix.toUpperCase()}`,
        {
      ...
      const publicDefaultRoute = new ec2.CfnRoute(
        this,
        `PublicDefaultRoute${azSuffix.toUpperCase()}`,
        {
          destinationCidrBlock: '0.0.0.0/0',
