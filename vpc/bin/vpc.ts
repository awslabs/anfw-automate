...
async function main() {
  ...
  try {
    ...
    const loadedConfig: StackConfig = await configManager.loadConfig('vpc', STAGE);
    ...
    // eslint-disable-next-line no-console
    console.log(`✅ Configuration loaded successfully for vpc module in stage '${STAGE}'`);
    // eslint-disable-next-line no-console
    ...
    if (error instanceof ConfigurationError) {
      ...
      // eslint-disable-next-line no-console
      console.error(`   - Verify SSM parameter: /anfw-automate/${STAGE}/vpc/config`);
      // eslint-disable-next-line no-console
      console.error(`   - Or check files: conf/${STAGE}.json and vpc/conf/${STAGE}.json`);
    } else {
      // eslint-disable-next-line no-console
